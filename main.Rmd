...
cleveland <-
  ...
  ungroup() %>%
  # arrange(desc(zori_avg - hud_avg)) %>%
  # slice_head(n = 5) %>%
  slice_max(order_by = zori_avg - hud_avg, n = 5, with_ties = FALSE) %>%
  pivot_longer(cols = c(zori_avg:hud_avg),
               names_to = "source",
               values_to = "rent") %>%
  ggplot(aes(x = reorder(RegionName, rent), y = rent)) +
  geom_line(aes(group = RegionName), size = 1) +
  # ggplot(aes(x = RegionName)) +
  # geom_line(aes(y = hud_avg, yend = zori_avg), size = 2) +
  # geom_point(aes(y = zori_avg), size = 5, color = "blue") +
  # geom_point(aes(y = hud_avg), size = 5) +
