...
# downloaded to flat file to reduce inconsistencies from API changes
# flat file not currently adjusted to base year
# for CUSRxxxxxxx and CUUSSxxxxx variables

show(blscrapeR::bls_api("CUSR0000SA0"))

inflation_table <- read_tsv("./Data/BLS/cu.data.1.AllItems.txt") %>%
  mutate(year = as.numeric(year),
         adj_value = value) %>%
  filter(year %in% c(2014:2020))
...
# adjust = function(df, year) {
#   inflation_values = inflation_table %>% filter(year == year) %>% pull(adj_value)
#   # show(df)
#   ret = df %>% select(hud_avg) / inflation_values
#   ret = ret %>% mutate(year = year)
#   ret
# }
# 
# data <- zori %>%
...
#   rename(hud_avg = br_avg, zori_avg = rent_avg) %>%
#   # Adjust dollar values to 2019 dollars
#   mutate(hud_avg = ifelse(is.na(hud_avg), 
#                           median(hud_avg, na.rm = TRUE), 
#                           hud_avg)) %>%
#   group_by(year) %>%
#   group_modify(~ adjust(.x, .x$year))
