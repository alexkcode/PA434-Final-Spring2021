...
zcta <- zctas(cb = T, starts_with = starts_with) %>%
  ...
  st_transform(3435) %>% # Without this transformation to ILNAD83 the map looks weird
  st_filter(cook_county) # There are ZIP codes that fall outside of Cook county
                         # that make the map less easy to follow. 
...
map_anim <- data %>%
  ...
  labs(title = "Year: {current_frame}",
       ...
       fill = "ZORI - HUD") + # Adds the year value to the title through interpolation
