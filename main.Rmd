...
summary_table <- function(data,
  ...
  # "rows" means variable names are their own columns
  x = "Statistic"
  y = ".value"
  ...
  data %>%
    ...
    pivot_longer(cols = everything(),
                 names_to = c(x, y), # You can switch these two values around to transpose the table
                 names_pattern = "(.*)_(.*)") %>%
...
### coor plot
### diff and hh_income
# TODO: Do the same with gini_index and percent_black_pop - EJ
...
```{r}
plot3 <- data_2014 %>%
  ggplot(aes(x = median_hh_income, y = gini_index)) +
  geom_jitter() +
  geom_smooth(method = "lm") +
  labs(title = "Gini Index Compared with Household Income, 2014",
      x = "Median Household Income",
      y = "Gini Index") +
  scale_x_continuous(limits = c(0, 190000)) + 
  theme_minimal()
...
plot4 <- data_2019 %>%
  ...
  geom_jitter() +
  geom_smooth(method = "lm") +
  labs(title = "Gini Index Compared with Household Income, 2019",
      x = "Median Household Income",
      y = "Gini Index") +
  scale_x_continuous(limits = c(0, 190000)) + 
  theme_minimal()
